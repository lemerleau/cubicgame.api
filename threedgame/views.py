...
def saveCubeData(request):
    ...
    for move in listOfMoves_in:
        rps = []
        for rp in move:
            rp = RingPosition.objects.all().filter(
                label=rp['binary'], color=color_codes[rp['color']])
            if len(rp) > 0:
                rps += [rp[0]]
            else:
                rp = RingPosition(label=rp['binary'],
                                  color=color_codes[rp['color']])
                rp.save()
                rps += [rp]
        m = Move()
        m.save()
...
def getAllCubeData(request):
    ...
    serializer = CubeDataSerializer(
        cubedata, many=True, context={
            'request': request})
    return JsonResponse(serializer.data, safe=False)
